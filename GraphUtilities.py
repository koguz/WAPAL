...
def get_connected_vertices(vertices, ratio):
    ...
    if number_of_samples < 0:
        number_of_samples = 0
    # the line below first takes the difference of cluster combinations from the connected set, so that we can
    # sample from another set of combinations. then samples from them as many as the number of samples and joins
    # them with the connected set again. sample returns a list, that's why it is converted to set...
    connected_set = list(
        set(sample(vertex_combinations.difference(connected_set), number_of_samples)).union(connected_set))
